  ...
  return new Promise((resolve, reject) => {
    ...
    axios.get(encodeURI(formatted_search_url), {
      ...
      final_data.results = titles.map((title, index) => {
        return { title : title || 'N/A', description: descriptions[index] || 'N/A', url: urls[index] || 'N/A', favicons: { high_res: `https://api.faviconkit.com/${new URL(urls[index]).hostname}/192`, low_res: `https://www.google.com/s2/favicons?sz=64&domain_url=${new URL(urls[index]).hostname}` } };
      });
      ...
      // This usually appears when you search for an animal.
      ...
      resolve(final_data);
    }).catch((err) => reject({ error: err.toJSON().message }));
  });
}
