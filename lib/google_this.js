...
const search = (query, options = {
  page: 0, debugging: false
}) => {
  debugging = options.debugging;

  var page = options.page * 10;
  var search_query = query.trim().split(/ +/).join("+").toLowerCase();
  var formatted_search_url = `${google_base_url}search?q=${search_query}&aqs=chrome..69i57.1685j0j4&client=ms-android-motorola-rev2&sourceid=chrome-mobile&ie=UTF-8${page != 0 ? `&start=${page}`: ``}`;
  ...
  return new Promise((resolve, reject) => {
    ...
    .then((res) => {
      ...
      **/
      ...
      var knowledge_panel_desc = $(utils.kno_panel_description_selector)[0];
      ...
      var born_date = $(utils.kno_panel_born_date_selector)[0];
      if (born_date) {
      ...
      var song_lyrics = $(utils.kno_panel_song_lyrics_selector)[0];
      if (song_lyrics) {
      ...
      // Games:
      var videogame_release_date = $(utils.kno_panel_videogame_release_date_selector)[0];
      ...
      var developer = $(utils.kno_panel_videogame_developer_selector)[0];
      if (developer) {
        final_data.knowledge_panel.developer = $(developer).text();
      }

      var videogame_composer = $(utils.kno_panel_videogame_composer_selector)[0];
      if (videogame_composer) {
        final_data.knowledge_panel.composer = $(videogame_composer).text();
      }

      var series = $(utils.kno_panel_webfacts_series_selector)[0];
      if (series) {
      ...
      var budget =
      $(utils.kno_panel_film_budget_selector) || undefined;
      if (budget && budget != "") {
      ...
      var google_users_rating = $(utils.kno_panel_film_googleusers_rating_selector)[0];
      ...
      $(utils.kno_panel_film_ratings_selector[0]).each(function (i, el) {
        final_data.knowledge_panel.ratings =
        final_data.knowledge_panel.ratings || [];
        var name = $($(utils.kno_panel_film_ratings_selector[1])[i]).attr('title');
        var rating = $(el).text();
        final_data.knowledge_panel.ratings.push({
          name: name,
          rating: rating,
      ...
      // Organizations:
      var organization_ceo = $(utils.kno_panel_organization_ceo_selector)[0];
      if (organization_ceo || organization_ceo === "") {
        final_data.knowledge_panel.ceo = $(organization_ceo).text().split(':')[1].trim();
      }

      var founded_date = $(utils.kno_panel_organization_founded_date_selector)[0];
      if (founded_date) {
        final_data.knowledge_panel.founded = $(founded_date).text().split(':')[1].trim();
      }

      var parent_organization = $(utils.kno_panel_organization_parent_selector)[0];
      if (parent_organization) {
        final_data.knowledge_panel.parent_organization = $(parent_organization).text().split(':')[1].trim();
      }

      var headquarters = $(utils.kno_panel_organization_headquarters_selector)[0];
      if (headquarters) {
        final_data.knowledge_panel.headquarters = $(headquarters).text().split(':')[1].trim();
      }

      var subsidiaries = $(utils.kno_panel_organization_subsidiaries_selector)[0];
      if (subsidiaries) {
        final_data.knowledge_panel.subsidiaries = $(subsidiaries).text().split(':')[1].trim();
      }

      var founders = $(utils.kno_panel_organization_founders_selector)[0];
      if (founders) {
        final_data.knowledge_panel.founders = $(founders).text().split(':')[1];
      }
      ...
      $($(utils.featured_snippet_title_selector[2])).text() || undefined;
      ...
      .join("\n")
      .trim():
      $(utils.featured_snippet_desc_selector[2])[0]
      ...
      if (word != '') {
        final_data.dictionary = {
          word: $(word).text(),
          phonetic: $(phonetic === '') ? 'n/a': $(phonetic).text(),
      ...
      $(utils.top_stories_snippet_selector).each(function (i, el) {
        ...
        var story_website = $(
          $(utils.top_stories_website_selector)[i]
        ).attr("alt");
        var story_snippet = $(el).text().trim();
        var story_url = $($(utils.top_stories_url_selector)[i]).attr("data-amp");
