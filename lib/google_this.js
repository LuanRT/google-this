...
function search (query, options = { ris: false, page: 0, match_all_images: false, debugging: false, additional_params: null }) {
  ...
  return new Promise((resolve, reject) => {
    debug('Making request to '+formatted_search_url);
    axios.get(formatted_search_url, {
      ...
      final_data.results = titles.map((title, index) => {
        return { title : title || 'N/A', description: descriptions[index] || 'N/A', url: urls[index] || 'N/A', favicons: { high_res: `https://api.faviconkit.com/${new URL(urls[index] || 'https://google.com').hostname}/192`, low_res: `https://www.google.com/s2/favicons?sz=64&domain_url=${new URL(urls[index] || 'https://google.com').hostname}` } };
      });
      ...
      if (unit_converter_input && unit_converter_output) {
        final_data.unit_converter = { input: unit_converter_input, output: unit_converter_output, formula: unit_converter_formula };
      } else if (currency_converter_input && currency_converter_output) {
        final_data.unit_converter = { input: { name: input_currency_name, value: currency_converter_input }, output: { name: output_currency_name, value: currency_converter_output } };
      }
      ...
      if (weather_location && weather_forecast) {
        final_data.weather = { location: weather_location, forecast: weather_forecast, precipitation, humidity: air_humidity, temperature: temperature+' °C', wind: wind_speed };
      }
      ...
      if (location_title && location_distance) {
        final_data.location = { title: location_title, distance: location_distance, map: 'https://google.com'+location_image };
      }
      ...
      if (word) {
        final_data.dictionary = { word: word || 'N/A', phonetic: phonetic || 'N/A', audio: audio ? `https:${audio}`: 'N/A', };
      ...
      if (source_text.length > 0) {
        final_data.translation = { source_language, target_language, source_text, target_text };
      }
...
function getTopNews () {
  ...
  return new Promise((resolve, reject) => {
    axios.get(formatted_url, {
      headers: {
        "User-Agent": userAgents.MOBILE,
        "Referer": "https://www.google.com/news",
        "Accept": "text/html",
        "Accept-Language": "en-US",
        "Accept-Encoding": "gzip",
      },
