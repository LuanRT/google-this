...
function search(
  ...
  const formatted_search_url = options.ris
    ? `${urls.w_google_base_url}searchbyimage?image_url=${query}`
    : `${
        urls.google_base_url
      }search?q=${search_query}&aqs=chrome..69i57.1685j0j4&client=ms-android-motorola-rev2&sourceid=chrome-mobile&ie=UTF-8${
        options.safe ? "&safe=active" : ""
      }${page != 0 ? `&start=${page}` : ""}`;
      ...
      .then((res) => {
        ...
        if (song_ft_artist[0]) {
          ...
          song_ft_artist.map((i, el) => {
            final_data.knowledge_panel.featured_artist.push(
              $(el).text().trim()
            );
          });
        }
        ...
        const song_release_date = $(
          selectors.kno_panel_song_release_date_selector
        )[0];
        if (song_release_date) {
          final_data.knowledge_panel.release_date = $(song_release_date)
            .text()
            .split(":")[1]
            .trim();
        }
        ...
        // Games:
        const videogame_release_date = $(
          selectors.kno_panel_videogame_release_date_selector
        )[0];
        ...
        const developer = $(
          selectors.kno_panel_videogame_developer_selector[0]
        );
        if (developer[0]) {
          ...
          const developer = $(
            selectors.kno_panel_videogame_developer_selector[1]
          );
          final_data.knowledge_panel.developer = [];
          developer.map((i, el) => {
            final_data.knowledge_panel.developer.push($(el).text());
          });
          if (final_data.knowledge_panel.developer.length == 0)
            delete final_data.knowledge_panel.developer;
        }

        const publisher = $(
          selectors.kno_panel_videogame_publisher_selector[0]
        );
        if (publisher[0]) {
          ...
          const publisher = $(
            selectors.kno_panel_videogame_publisher_selector[1]
          );
          final_data.knowledge_panel.publisher = [];
          publisher.map((i, el) => {
            final_data.knowledge_panel.publisher.push($(el).text());
          });
          if (final_data.knowledge_panel.publisher.length == 0)
            delete final_data.knowledge_panel.publisher;
        }
        ...
        if (designer[0]) {
          ...
          const designer = $(
            selectors.kno_panel_videogame_designer_selector[1]
          );
          final_data.knowledge_panel.designer = [];
          designer.map((i, el) => {
            final_data.knowledge_panel.designer.push($(el).text());
          });
          if (final_data.knowledge_panel.designer.length == 0)
            delete final_data.knowledge_panel.designer;
        }

        const videogame_composer =
          $(selectors.kno_panel_videogame_composer_selector[0])[0] ||
          $(selectors.kno_panel_videogame_composer_selector[1])[0];
        ...
        const platforms =
          $(selectors.kno_panel_videogame_platforms_selector) || undefined;
        ...
        const award_nominations = $(
          selectors.kno_panel_award_nominations_selector
        );
        ...
        const box_office =
          $(selectors.kno_panel_film_boxoffice_selector) || undefined;
        ...
        const film_release_date = $(
          selectors.kno_panel_film_release_date_selector
        )[0];
        if (film_release_date) {
          final_data.knowledge_panel.release_date = $(film_release_date)
            .text()
            .split(":")[1]
            .trim();
        }

        const google_users_rating = $(
          selectors.kno_panel_film_googleusers_rating_selector
        )[0];
        ...
        $(selectors.kno_panel_film_ratings_selector[0]).each(function (i, el) {
          ...
          const name = $(
            $(selectors.kno_panel_film_ratings_selector[1])[i]
          ).attr("title");
        ...
        // Organizations:
        const organization_ceo = $(
          selectors.kno_panel_organization_ceo_selector
        )[0];
        ...
        const founded_date = $(
          selectors.kno_panel_organization_founded_date_selector
        )[0];
        if (founded_date) {
        ...
        const parent_organization = $(
          selectors.kno_panel_organization_parent_selector[0]
        )[0];
        if (parent_organization) {
          final_data.knowledge_panel.parent_organization = (
            $(parent_organization).text().split(":")[1] ||
            $($(selectors.kno_panel_organization_parent_selector[1])).text()
          ).trim();
        }

        const headquarters = $(
          selectors.kno_panel_organization_headquarters_selector
        )[0];
        if (headquarters) {
        ...
        const subsidiaries = $(
          selectors.kno_panel_organization_subsidiaries_selector
        )[0];
        if (subsidiaries) {
        ...
        const founders = $(
          selectors.kno_panel_organization_founders_selector
        )[0];
        if (founders) {
          final_data.knowledge_panel.founders = $(founders)
            .text()
            .split(":")[1]
            .trim();
        }
        ...
        // Animals:
        const animal_preview = getStringBetweenStrings(
          data,
          "source src\\x3d\\x22",
          ".mp4"
        );
        ...
        const featured_snippet_title =
          ...
          $($(selectors.featured_snippet_title_selector[2])).text() ||
          undefined;
        ...
        const featured_snippet =
          ...
          $(selectors.featured_snippet_desc_selector[0])[0]
            ...
            ? $($(selectors.featured_snippet_desc_selector[2])[0]).text()
            : undefined;
        ...
        $(selectors.top_stories_snippet_selector).each(function (i, el) {
          ...
          const story_website = $(
            $(selectors.top_stories_website_selector)[i]
          ).attr("alt");
          const story_snippet = $(el).text().trim();
          const story_url = $($(selectors.top_stories_url_selector)[i]).attr(
            "data-amp"
          );
...
function image(
  ...
  var formatted_search_url = `${urls.gis_base_url}search?tbm=isch${
    options.safe ? "&safe=active" : ""
  }&q=${search_query}`;
