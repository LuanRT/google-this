  ...
  return new Promise((resolve, reject) => {
    axios
      ...
      .then((res) => {
        ...
        /* Website path (Can be kinda buggy sometimes so I'll leave it disabled for now.)  
        $('span[class="Zu0yb UGIkD qzEoUe"]').each(function(i, el) {
           paths.push($(el).text());
        });
        */
        ...
        $('g-img[class="BA0A6c"] > img').each(function (i, el) {
          if (el.attribs["data-src"]) {
            ...
            final_data.knowledge_panel.images.push(el.attribs["data-src"]);
          }
        });
        ...
        /* Featured Snippet */
        var featured_snippet_title = $('div[class="co8aDb gsrt"]')
          ? $($('div[class="co8aDb gsrt"]')).text()
          : undefined;
        ...
        var featured_snippet = $('ol[class="X5LH0c"]')[0]
          ? $($('ol[class="X5LH0c"]')[0])
              .html()
              .replace(/<\/li>/g, "")
              .replace(/<b>/g, "")
          ...
          ? $($('div[data-attrid="wa:/description"]')[0]).text()
          : undefined;
        ...
        if (
          descriptions.length >
          !paths.length >
          !(featured_snippet_title && featured_snippet
            ? urls.slice(1).length
            : urls.length)
        ) {
          debug("[INFO] Everything seems to be ok.");
        } else {
          debug("[INFO] There's something off..");
        }
