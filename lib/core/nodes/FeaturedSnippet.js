...
class FeaturedSnippet {
  ...
  constructor($) {
    const featured_snippet_title =
      $(Constants.SELECTORS.FEATURED_SNIPPET_TITLE[0]).prop('innerText') ||
      $(Constants.SELECTORS.FEATURED_SNIPPET_TITLE[1]).prop('innerText') ||
      $(Constants.SELECTORS.FEATURED_SNIPPET_TITLE[2]).prop('innerText') || '';
    ...
    const featured_snippet = Constants.SELECTORS.FEATURED_SNIPPET_DESC.map((selector) => {
      if ($(selector)[0] && selector != Constants.SELECTORS.FEATURED_SNIPPET_DESC[2]) {
        ...
        return $(selector).prop('innerText') || '';
      }
