...
class Weather {
  ...
  constructor($, data) {
    const weather_location = $(Constants.SELECTORS.WEATHER_LOCATION).prop('innerText') || '';
    const weather_forecast = $(Constants.SELECTORS.WEATHER_FORECAST).prop('innerText') || '';
    const precipitation = $(Constants.SELECTORS.PRECIPITATION).prop('innerText') || '';
    const air_humidity = $(Constants.SELECTORS.AIR_HUMIDITY).prop('innerText') || '';
    const temperature = $(Constants.SELECTORS.TEMPERATURE).prop('innerText') || '';
    const wind_speed = $(Constants.SELECTORS.WIND_SPEED).prop('innerText') || '';
