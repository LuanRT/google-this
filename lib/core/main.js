 ...
 * @param {Axios.AxiosInstance} [options.axios_instance] - Axios instance that will be used to make the request.
 */
async function search(query, options = {}) {
  ...
  if (options.axios_instance && !(options.axios_instance instanceof Axios.AxiosInstance)) throw new Error('axios_instance must be an instance of AxiosInstance');
  const axios_instance = options.axios_instance || Axios;

  if (typeof query === 'object' && ris) {
    response = await uploadImage(query, axios_config, axios_instance);
  } else {
    ...
    response = await axios_instance.get(url, {
...
async function uploadImage(buffer, axios_config, axios_instance) {
  const form_data = new FormData();

  form_data.append('encoded_image', buffer);

  const response = await axios_instance.post(`${Constants.URLS.GIS}searchbyimage/upload`, form_data, {
    headers: {
...
async function image(query, options = {}) {
  ...
  const response = await axios_instance.post(`${Constants.URLS.W_GOOGLE}_/VisualFrontendUi/data/batchexecute`, form_data, {
    params: {
      'rpcids': 'HoAMBc',
      'source-path': '/search',
