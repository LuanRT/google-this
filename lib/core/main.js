...
async function search(query, options = {}) {
  ...
  const did_you_mean = $(Constants.SELECTORS.DID_YOU_MEAN).prop('innerText') || '';
...
async function getTopNews(language = 'en', region = 'US') {
  ...
  const headline_stories_publishers = $(Constants.SELECTORS.PUBLISHER).map((i, el) => $(el).prop('innerText')).get();
  const headline_stories_imgs = $(Constants.SELECTORS.STORY_IMG).map((i, el) => $(el).attr('src')).get();
  const headline_stories_time = $(Constants.SELECTORS.STORY_TIME).map((i, el) => $(el).prop('innerText')).get();

  $(Constants.SELECTORS.STORY_TITLE).each((i, el) => {
    const headline_stories_title = $(el).prop('innerText') || '';
