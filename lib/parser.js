...
class Parser {
  ...
  getOrganicResults() {
    const titles = this.$(Constants.SELECTORS.TITLE).map((i, el) => {
      if (el.parent.attribs.style != '-webkit-line-clamp:2') // ignores ad titles
        return this.$(el.children).text().trim();
    }).get();
    const descriptions = this.$(Constants.SELECTORS.DESCRIPTION).map((i, el) => {
  ...
  getKnowledgeGraph(data) {
    ...
    const song_lyrics = this.$(Constants.SELECTORS.KNO_PANEL_SONG_LYRICS).map((i, el) => this.$(this.$(el).html().replace(/<\/span><\/div><div jsname="u8s5sf" class="ujudub"><span jsname="ys01ge">/g, '\n\n').replace(/<br>/g, '\n')).text()).get();
    ...
    const animal_preview = Utils.getStringBetweenStrings(this.data, 'source src\\x3d\\x22', '.mp4');
  ...
  getFeaturedSnippet() {
    const featured_snippet_title = this.$(Constants.SELECTORS.FEATURED_SNIPPET_TITLE[0]).text() || this.$(Constants.SELECTORS.FEATURED_SNIPPET_TITLE[1]).text() || this.$(Constants.SELECTORS.FEATURED_SNIPPET_TITLE[2]).text() || undefined;
    ...
    const featured_snippet = Constants.SELECTORS.FEATURED_SNIPPET_DESC.map((selector) => {
      if (this.$(selector)[0] && selector != Constants.SELECTORS.FEATURED_SNIPPET_DESC[2]) {
        const text = this.$(selector).html().replace(/<\/li>|<\/b>|<b>/g, '').replace(/&amp;/g, '&').split('<li class="TrT0Xe">').join('\n').trim();
        return text;
  ...
  getTopStories() {
    // Removes unnecessary text from the short description
    ...
    const top_stories_descriptions = Constants.SELECTORS.TOP_STORIES_DESCRIPTION.map((selector) => this.$(selector).map((i, el) => this.$(el).text().slice(1)).get()).filter((descs) => descs.length > 0)[0];
  ...
  getPaa() {
    let people_also_ask = [];
    Constants.SELECTORS.PAA.forEach((item) => this.$(item).each((i, el) => people_also_ask.push(this.$(el).text())));
  ...
  correctFuzzyData(titles, descriptions, urls) {
    // Corrects wrongly parsed data, this doesn't often happen though.
